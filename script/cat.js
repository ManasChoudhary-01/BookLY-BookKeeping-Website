...
function fetchData(option, element) {

    const allOptions = document.querySelectorAll('.med4');
    allOptions.forEach(opt => opt.classList.remove('active1'));

    element.classList.add('active1');

    const workElement = element.closest('section').querySelector('.work');
    workElement.innerHTML = '';
   
    fetch(`https://book-finder1.p.rapidapi.com/api/search?${option}`, {
        "method": 'GET',
        "headers": {
    ...
    for (let i = 0; i < 8 && i < response.results.length; i++) {
        ...
        workElement.appendChild(bookDetailsElement);
